...
Top line, left to right: a lavender session-name pill (terminal icon); while the focused pane is ssh'd, a red `SSH:` target pill (server icon, `#{window_name}`, which the zsh `ssh()` wrapper already renames to `ssh:<target>` on connect, so no separate detection mechanism was needed — see Window title above for why that name no longer also drives the outer window title); a green cwd pill (folder icon, `#{pane_current_path}`). Right-aligned: a blue date+time pill (clock icon). Bottom line: the window list, using the same session-pill styling for the active window. Every pill is a single color zone — `#[fg=accent]#[bg=bar-bg]<capL>#[fg=crust]#[bg=accent] icon label #[fg=accent]#[bg=bar-bg]<capR>` (`crust`, near-black, for contrast against the saturated accent fill) — deliberately *not* the two-tone "bright icon chip + muted label chip" treatment that was the original design intent, because that construction is real-pty-confirmed to break tmux's own rendering (see below). Icons are Font Awesome codepoints from the Nerd Fonts symbol set (terminal U+F120, server U+F233, folder U+F07B, clock U+F017) verified against the canonical `nerd-fonts` `glyphnames.json`, not typed from memory — same Nerd-Font-required caveat as the pill caps (can't visually verify actual glyph rendering from a sandbox with no patched font). Pills use the U+E0B4/U+E0B6 rounded-cap glyphs from the original Powerline Extra Symbols block — the oldest, most stable part of the Nerd Font symbol set, present in every variant, chosen over a newer icon set specifically to avoid a tofu-box risk this can't visually verify from a sandbox. Each cap's `fg` is the pill's own fill color and its `bg` matches whatever's behind it, which is what reads as a curve instead of a hard edge. Both lines are Mocha/Frappe-aware via the same SSH-detection ternary as `status-style` (see comment there for the `-F`-freezes-the-value pitfall) — built as two complete literal strings picked by one outer ternary rather than interleaving small ternaries, so the SSH condition is written once per option instead of repeated per cap. Each `#[...]` block sets exactly one attribute (`#[fg=X]#[bg=Y]`, never a comma-joined `#[fg=X,bg=Y]`): a comma there would need escaping as `\,` to survive being embedded inside the outer `#{?cond,yes,no}` ternary, and that shipped once already — real-pty capture (`script`, not `show-options`, which only proves the stored value, not the render) showed tmux failing to unescape it before the style parser ran, leaking `fg=...`/`bg=...` as literal on-screen text. Single-attribute blocks never put a comma inside a ternary branch, so there's nothing to escape.

**Why pills are single-zone, not two-tone** (icon chip + separately-colored muted label chip): built and real-pty-tested that version first, since it's what was actually asked for. With 3 pills on the left (the ssh branch) plus a two-tone pill on the right, `%a` (the clock's day-of-week) and everything before it in the right-aligned pill silently vanished from the render -- not a truncation by `status-right-length` (raising it to 200 didn't help) and not a terminal-width overflow (reproduced identically at 500 columns). Bisected via real-capture A/B tests (same methodology as the `\,`-escaping bug): the two-tone construction alone works fine, and the SSH-detection ternary alone works fine, but combining ~3+ two-tone pills inside a `#{?cond,...}` branch *and* a two-tone pill on the other, right-aligned side breaks -- and it's specifically the added `│` (U+2502) divider glyph between a pill's icon and label zones that tips it over: the same 3-pill-plus-clock layout with that one character removed (icon and label sharing one zone, no divider) renders correctly, confirmed 3/3 on repeated runs. Likely cause: `│` renders via legacy VT100 ACS charset translation (`\x0e`+`x`+`\x0f` in the raw capture, not a literal UTF-8 box-drawing byte), and tmux's internal width accounting for that translated form seems to misbehave specifically inside a `#{?...}` ternary once combined with an aligned-right block elsewhere on the same line -- not confirmed as a documented tmux bug, just empirically triggered and worked around. If a future edit reintroduces a divider character between a pill's icon and label, retest exactly this way (real `script` capture of the ssh-branch, 3-pill worst case, not just `show-options`) before trusting it.
