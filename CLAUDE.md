...
`tmux/.config/tmux/tmux.conf` (XDG path, tmux 3.1+, but `pane-border-lines rounded` needs 3.3+ — see below). Default prefix (`Ctrl-b`), vi copy-mode, mouse on. `Ctrl-h/j/k/l` move between tmux panes and Neovim splits seamlessly (mirrors the Neovim `<C-hjkl>` window-nav keymaps via a `pane_tty`/process check — no plugin needed). Copy-mode `y` uses tmux's OSC 52 clipboard relay (see Clipboard above) rather than a `wl-copy` pipe, so it works the same locally and over SSH.

**Status bar**: rounded powerline-style pills (session name on the left, clock on the right, active window tab styled the same way) using the U+E0B4/U+E0B6 rounded-cap glyphs from the original Powerline Extra Symbols block — the oldest, most stable part of the Nerd Font symbol set, present in every variant, chosen over a newer icon set specifically to avoid a tofu-box risk this can't visually verify from a sandbox. Each cap's `fg` is the pill's own fill color and its `bg` matches whatever's behind it, which is what reads as a curve instead of a hard edge. Both pills and the active window tab are Mocha/Frappe-aware via the same SSH-detection ternary as `status-style` (see comment there for the `-F`-freezes-the-value pitfall) — built as two complete literal strings picked by one outer ternary rather than interleaving small ternaries, so the SSH condition is written once per option instead of repeated per cap. `pane-border-lines rounded` softens the pane dividers too, but needs tmux 3.3+; comment it out if `tmux` refuses to start citing that line (unlikely on either real target machine — Arch is rolling-release, Homebrew stays current).
