...
On desktop, `dev`'s zsh function spawns a new, untagged `foot` window to run `tmux-dev` in (`( foot tmux-dev "${1:-$PWD}" & ) 2>/dev/null`) rather than running it inline in the calling shell. `tmux-dev` ends by attaching or `switch-client`-ing the *calling* client onto the `dev-<dir>` session, so running it inline would hijack whichever window `dev` was typed into — including the `main`-app-id window `Mod+Return` manages, silently switching it away from `main` and breaking that binding's "always shows `main`" contract. A fresh window sidesteps this: it gets its own tmux client, leaving the window `dev` was launched from untouched. The subshell-backgrounding is the same job-control-message-suppression trick as the git-fetch hook above.
